...
bool GameScene::init()
{
	...
	auto fireToZombieListener = EventListenerCustom::create("fire", [&](EventCustom* e) {
		...
		if (vTouch->x < hero->getPositionX()) //¡÷¿Œ∞¯ øﬁ¬ ¿ª ΩÓ∏È
			boss->damaged(100); //∫∏Ω∫ø°∞‘ µ•πÃ¡ˆ 100
...
void GameScene::menuItemCallBack(Ref * sender)
{
	...
	{
		item->setTag(0);
		item->setColor(Color3B::BLACK);	//≤Ù∞Ì ªˆ±Ú black¿∏∑Œ
...
bool GameScene::OnContactBegin(PhysicsContact& contact)
{
	...
	if (character[0]->getCurrentState().compare("die") == 0 ||
		...
		return false; //µ— ¡ﬂ «—∏Ì¿Ã∂Ûµµ ¡◊¿∫ªÛ≈¬∏È √Êµπ X
